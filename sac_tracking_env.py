...
class Environment():
    ...
    def __init__(
        ...
        img, true_density, mask = list(torch.load(self.img_files[0], weights_only=True).values())
        ...
        self.paths = [torch.cat((point - step*self.step_size, point)).to(device=DEVICE) for point, step in zip(self.paths, last_step)]
    ...
    def get_reward(self, matching_precision=None, self_overlap=None, bifurcate=False, terminate=False, verbose=False, sigmaf=10.0, sigmab=0.4):
        ...
        if terminate:
            travel_dist = torch.linalg.norm(self.paths[self.head_id][-1] - self.paths[self.head_id][0])
    ...
    def step(self, action, max_paths=100, verbose=False):
        ...
        if terminate_path:
        ...
        elif choice==0: # otherwise take a step
            ...
            observation = self.get_state() # observation = (patch, last_step)
    ...
    def reset(self):
        ...
        img, true_density, mask = list(torch.load(self.img_files[img_idx], weights_only=True).values())
        self.img = Image(img.to(device=DEVICE))
        self.mask = mask
        self.true_density = Image(true_density.to(device=DEVICE))        # start with next seed point
        seed_id = self.n_resets % len(self.seeds)
        center = torch.Tensor(self.seeds[seed_id])
        offsets = torch.randn((self.n_seeds, 3))
        offsets /= torch.sum(offsets**2, dim=1)**0.5
